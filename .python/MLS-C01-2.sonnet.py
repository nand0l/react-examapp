...
ExamID = input("ExamCode: ").upper() or "MLS-C01-V2.1"
#base_path = r'C:\Users\wnl\OneDrive\Documents\__SkillSoft\__AWS\MachineLearning\mls-c01.dev'
prompt1_file_path= os.path.join(base_path, r'.python',prompt1_filename)
prep_folder = 'prep'
...
def reformat_json(input_json, ExamID):
    ...
    if isinstance(data['PossibleAnswers'], str):
        data['PossibleAnswers'] = split_possible_answers(
...
def process_input_json(ExamID, input_json):
    ...
    try:
        ...
        output_file = save_to_file_and_dynamodb(
            answer_output, ExamID, answer_output['QuestionID'])
...
def invoke_bedrock_model(prompt):
    ...
    response = client.invoke_model(
        contentType='application/json', body=input_data_json, modelId=model_id)
    ...
    for filename in os.listdir(full_path):
        if filename.endswith('.md'):
            ...
            with open(file_path, 'r') as file:
                ...
                answer_output, output_file = process_input_json(
                    ExamID, content_text)
                if answer_output:
